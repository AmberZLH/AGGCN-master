...
def RNNReLUCell(input, hidden, w_ih, w_hh, w_ah, b_ih=None, b_hh=None, b_ah = None):
    hy = F.relu(F.linear(input, w_ih, b_ih) + F.linear(hidden, w_hh, b_hh) + F.linear(input, w_ah, b_ah))
    return hy


def RNNTanhCell(input, hidden, w_ih, w_hh, w_ah, b_ih=None, b_hh=None, b_ah = None):
    hy = torch.tanh(F.linear(input, w_ih, b_ih) + F.linear(hidden, w_hh, b_hh) + F.linear(input, w_ah, b_ah))
    return hy
