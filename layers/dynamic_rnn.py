...
class DynamicLSTM(nn.Module):
    ...
    def forward(self, x, x_len, aspect ,aspect_double_idx, aspect_len, h0=None):
        """
        ...
        aspect_unsort_idx = torch.argsort(aspect_double_idx).long()
        aspect_len = aspect_len.cpu().numpy()
        aspect = aspect[aspect_double_idx.long()]

        """pack"""
        x_emb_p = torch.nn.utils.rnn.pack_padded_sequence(x, x_len, batch_first=self.batch_first)
        aspect_emb_p = torch.nn.utils.rnn.pack_padded_sequence(aspect, aspect_len, batch_first=self.batch_first)
